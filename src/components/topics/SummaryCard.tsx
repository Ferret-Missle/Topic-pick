import { Sparkles, TrendingUp, TrendingDown, Minus, Globe } from 'lucide-react'
import type { Topic } from '../../types'

interface Props { topic: Topic }

const sentimentConfig = {
  positive: { label: '好調', color: 'text-daily', bg: 'bg-daily/10 border-daily/20' },
  negative: { label: '悪化', color: 'text-danger', bg: 'bg-danger/10 border-danger/20' },
  neutral:  { label: '中立', color: 'text-text-muted', bg: 'bg-bg-surface3 border-border' },
    ...
    <div className="relative bg-gradient-to-br from-bg-surface2 to-bg-surface rounded-2xl border border-border p-5 overflow-hidden">
      ...
      <div className="flex items-start gap-3 mb-4 relative">
        ...
        <div className="flex-1">
          <h2 className="font-display font-bold text-lg text-text leading-tight">{topic.name}</h2>
          {topic.description && (
            <p className="text-xs text-text-muted mt-0.5 line-clamp-1">{topic.description}</p>
          )}
        </div>
        ...
        <div className="flex flex-col items-end gap-1.5">
          {sentiment && (
            <span className={`px-2.5 py-1 text-xs font-semibold rounded-full border ${sentiment.bg} ${sentiment.color}`}>
              {sentiment.label}
            </span>
          )}
          {scale && (
      ...
      <div className="relative">
        {topic.summary ? (
          ...
          <div className="space-y-2">
            {[...Array(4)].map((_, i) => (
              <div key={i} className={`h-3 shimmer rounded-full ${i === 3 ? 'w-2/3' : 'w-full'}`} />
            ))}
          </div>
        )}
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-border/50 flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] text-text-muted uppercase tracking-wider">盛り上がり</span>
              <span className="text-[11px] font-mono font-semibold text-text">{td.buzzLevel}</span>
            </div>
            <div className="h-1 bg-bg-surface3 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-700 ${
                  td.buzzLevel > 70 ? 'bg-danger' : td.buzzLevel > 40 ? 'bg-warm' : 'bg-accent'
                }`}
          ...
          {weeklyChange !== undefined && (
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <span className={`text-sm font-mono font-semibold ${
                changePositive ? 'text-daily' : changeNegative ? 'text-danger' : 'text-text-muted'
              }`}>
          ...
          {td.keyThemes?.length > 0 && (
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <div className="flex gap-1">
                {td.keyThemes.slice(0, 2).map(theme => (
                  <span key={theme} className="px-1.5 py-0.5 text-[10px] bg-bg-surface3 border border-border rounded text-text-muted">
                    {theme}
                  </span>
                ))}
              </div>
