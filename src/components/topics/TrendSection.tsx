  ...
  if (!td) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-xs text-text-dim mt-1">情報を取得するとここに表示されます</p>
      </div>
    )
  }

  const segment = BUZZ_SEGMENTS.find(s => td.buzzLevel >= s.min && td.buzzLevel < s.max) ?? BUZZ_SEGMENTS[4]
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full bg-bg-surface3 border border-border text-text-muted`}>
              {segment.label}
            </span>
            <span className="font-display font-bold text-2xl text-text">{td.buzzLevel}</span>
        ...
        <div className="flex justify-between mt-1 px-0.5">
          {BUZZ_SEGMENTS.map(s => (
            <span key={s.label} className="text-[9px] text-text-dim">{s.label}</span>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="p-3 bg-bg-surface2 border border-border rounded-xl">
          <p className="text-[10px] text-text-dim uppercase tracking-wider mb-2">センチメント</p>
          <p className={`text-sm font-bold ${
            td.sentiment === 'positive' ? 'text-daily' :
            td.sentiment === 'negative' ? 'text-danger' :
            td.sentiment === 'mixed' ? 'text-warm' : 'text-text-muted'
          }`}>
            {{ positive: 'ポジティブ', negative: 'ネガティブ', neutral: '中立', mixed: '混在' }[td.sentiment]}
          </p>
        </div>

        {/* Scale */}
        <div className="p-3 bg-bg-surface2 border border-border rounded-xl">
          <p className="text-[10px] text-text-dim uppercase tracking-wider mb-2">関心規模</p>
          <p className="text-sm font-bold text-text">
            {{ niche: 'ニッチ', growing: '成長中', mainstream: '注目', viral: 'バイラル' }[td.interestScale]}
          </p>
        </div>

        {/* Weekly change */}
        <div className="p-3 bg-bg-surface2 border border-border rounded-xl">
          <p className="text-[10px] text-text-dim uppercase tracking-wider mb-2">前週比</p>
          {weeklyChange !== undefined ? (
            <div className="flex items-center gap-1">
              {weeklyChange > 0 ? <TrendingUp size={14} className="text-daily" /> : weeklyChange < 0 ? <TrendingDown size={14} className="text-danger" /> : null}
              <p className={`text-sm font-bold ${weeklyChange > 0 ? 'text-daily' : weeklyChange < 0 ? 'text-danger' : 'text-text-muted'}`}>
            ...
            {td.keyThemes.map((theme, i) => (
              <span
                ...
                className={`px-3 py-1.5 text-xs rounded-xl border font-medium transition-all ${
                  i === 0 ? 'bg-accent/10 border-accent/25 text-accent' :
                  i === 1 ? 'bg-warm/8 border-warm/20 text-warm' :
